/*
 * PRISMHORN — js13kGames 2026. Theme: "Unicorns and Rainbows".
 *
 * THE ONE IDEA: you never cast a spell. You cast a RAINBOW.
 *
 * A single shaft of dawn enters the valley from the east and strikes your horn.
 * It leaves as seven bands, and each band is a different verb — UNMAKE, FORGE,
 * WAKE, GROW, CHILL, SCRY, MEND. They leave in spectral order, in a rigid fan,
 * and you cannot fire one without firing all of them. GROW feeds your blooms;
 * it also feeds the blight standing next to them. MEND heals what you love and
 * what is eating it. The game is not aiming a weapon. It is arranging a valley
 * underneath a comb of light.
 *
 * Progress is the rainbow completing: you begin with three bands and empty slots
 * where the others belong, and every unlock fills one in — more power, and more
 * collateral, in the same purchase.
 *
 * ART: everything here is drawn procedurally. That is not a stylistic pose — at
 * a 13,312-byte cap a single 630x500 PNG is ~34x the entire budget, so generated
 * art is the only physically possible answer (wing CLAUDE.md 5b: this is the one
 * place the factory's art doctrine inverts, and the craft of doing it well IS the
 * competition).
 *
 * AUDIO: synthesised live through WebAudio. Our own IP, no samples, no licence,
 * no Gen-AI disclosure — procedural synthesis is expressly not generative AI.
 *
 * RULES COMPLIANCE (read off js13kgames.com/rules in-browser 2026-08-19):
 *   - no external resources of any kind; everything is in these two files.
 *   - save keys are namespaced `ph.` and localStorage.clear() is never called —
 *     every entry in the competition shares one origin, so clearing it would
 *     wipe other people's saves. That is a rule, not politeness.
 */
'use strict';
...
/* ===========================================================================
   1. THE BAND TABLE — the whole design hangs off this, so it is data.
   ---------------------------------------------------------------------------
   Spectral order is fixed: a band always leaves the horn from its own slot,
   whether or not you have unlocked it. That is why unlocking changes geometry
   you have already learned, instead of just adding a number.

   Each band names what it does to each of the four things in the valley.
   Read down a column and you can see the trap: GROW is your entire economy and
   it is also the fastest way to breed a monster.
   ========================================================================= */
const BANDS = [
  { n: 'UNMAKE', h: 354, say: 'dissolves the made',
    bloom: (e, p) => { e.hp -= p * 1.3; },
    /* Armour is FORGE's doing, and it is subtracted HERE — otherwise hardening
       a blight would be a purely cosmetic mistake, which is the worst kind. */
  ...
  { n: 'WAKE', h: 50, say: 'animates the dormant',
    ...
    /* Must outpace the cairn's own decay below, or WAKE can never wake
       anything. It could not, until the playtest measured 0.00 -> 0.00. */
...
/* You start with three: one killer, one economy, one utility — spread across
   the comb so the missing slots are visible from the first second. */
const START_BANDS = [0, 3, 4];

/* ===========================================================================
   2. UPGRADES — layered, and the band unlocks sit in the same shop so that
   "more rainbow" competes with "better rainbow" for the same Light.
   ========================================================================= */
const UPS = [
...
/* Unlock order: SCRY, FORGE, WAKE, then MEND last — MEND is the most
   double-edged band in the set, and buying it late is a real decision. It is
   also violet, so the rainbow finishes in the right place. */
const UNLOCK_ORDER = [5, 1, 2, 6];
const UNLOCK_COST = [55, 80, 115, 160];

/* ===========================================================================
   3. SAVE / RESUME. Death never restarts a run — it returns you to the Prism
   with every upgrade, every band and every mote of Light you had. (Wing rule:
   "save/resume on death, never a full restart".)
   ========================================================================= */
const KEY = 'ph.save';           // namespaced. js13k entries share one origin.
...
/* ===========================================================================
   4. TINY DETERMINISTIC RNG — so a wave's valley is the same valley if you
   fall and walk back into it. Rebuilding the world differently after a death
   would make "resume" a lie.
   ========================================================================= */
let seed = 1;
...
/* ===========================================================================
   5. AUDIO — a four-oscillator toybox. No samples, no library, our own IP.
   ========================================================================= */
...
/* An ambient bed: a slow pentatonic drift over a drone. Fifteen lines of music
   that never repeats exactly, and costs nothing to ship. */
...
/* ===========================================================================
   ...
   ========================================================================= */
let screen = 'title';
/* `horn` is seeded here, not left undefined until the first wave, because the
   TITLE SCREEN draws her too — and an undefined horn there is a crash, not a
   missing picture. buildWave() replaces it wholesale. */
...
/* The valley opens eastward as REACH grows. Seeding the far end on wave 1 would
   put blooms outside the base reach of every band, which reads as a broken game
...
function buildWave() {
  ...
  sun = { x: -10, y: H / 2 };
  vig = vigMax();
  /* Jam-length waves. A judge plays for two minutes, so a wave has to be a
     complete arc inside one of them. */
...
/* ===========================================================================
   8. THE COMB — the geometry that is the entire game.
   ---------------------------------------------------------------------------
   The incoming shaft's angle is decided by where you stand relative to the sun,
   so moving re-aims all seven bands at once. Each band leaves at its own
   spectral slot, offset from the centre of the fan. `spread` is the second
   axis of control: narrow stacks the whole spectrum onto one point, wide rakes
...
/** How strongly band `i` is falling on an entity, 0 when it misses. */
function lit(i, e) {
  ...
  const halfW = 6 + proj * .012;              // the fan thickens as it travels
  const edge = e.r + halfW;
  if (perp > edge) return 0;
  /* Softer at the rim and dimmer at range — so precision is rewarded. */
...
function step(dt) {
  ...
  /* FLARE — a spendable active, paid for in the same meter that keeps you
     alive. Panic is expensive. */
  flareOn = 0;
  if (K[' '] && vig > .06) { flareOn = 1; vig -= flareCost() * dt; if (Math.random() < dt * 6) SFX.flare(); }

  /* The shaft's source drifts, so the geometry is never static. */
  ...
  for (const e of ents) {
    ...
    if (e.grow >= 1) {
      /* Ripe: it bursts, pays out, and a new seed takes its place. A valley
         that empties is a valley with nothing to defend. */
  ...
  /* --- cairns: woken by WAKE, they burn blight near them ----------------- */
  for (const e of ents) {
  ...
  for (const e of ents) {
    ...
    if (e.chill > 0) continue;                      // stilled

    /* Hunt the nearest bloom; if the valley is bare, come for the horn. */
    let tgt = null, bd = 1e9;
    for (const b of ents) {
...
function fall() {
  /* Save/resume, the wing's standing rule: the Light you gathered is BANKED,
     the upgrades and bands are kept, and you walk back into the same valley.
     Nothing here restarts a run. */
  save(); SFX.fall();
  screen = 'fallen';
}

/* ===========================================================================
   10. THE PRISM (shop) — 1..5 buy upgrades, 6 buys the next band, ENTER goes on
   ========================================================================= */
function press(k) {
...
/* ===========================================================================
   11. PAINTING THE VALLEY — a layered backdrop, generated once per wave into an
   offscreen canvas so it costs nothing per frame. This is where the entry
   stops looking like a tech demo.
   ========================================================================= */
function paintValley() {
  ...
  /* Sky: a deep vertical wash, warmer at the horizon where the shaft enters. */
  ...
  /* An aurora — three broad translucent arcs, the theme's own colours. */
  ...
  /* Four ridges, back to front, each darker and rougher than the last. */
  for (let L = 0; L < 4; L++) {
    ...
    /* A rim of caught light on the crest — the cheapest way to make a
       silhouette read as a landscape rather than a shape. */
    g.strokeStyle = `hsla(${(base + 40) % 360},80%,${58 - L * 8}%,${.20 - L * .04})`;
    g.lineWidth = 1.6; g.stroke();
  }

  /* Ground haze so entities sit IN the valley rather than on top of it. */
...
/* ===========================================================================
   12. THE CREATURE — a real unicorn, drawn with curves, galloping on the air.
   The horn TIP is the origin, because the tip is the thing the player aims.
   ========================================================================= */
/**
 * The creature, in profile, facing east into the valley — head, neck, barrel,
 * croup and four galloping legs, with the HORN TIP at the origin because the
 * tip is the thing the player aims.
 *
 * ⚠️ REDRAWN 2026-08-21 after looking at the frames. The first version was a
 *    six-curve blob: the head was too small for the neck, the legs were stubs,
 *    and the horn was a thread. On a jam whose theme is literally "Unicorns and
 *    Rainbows" it read as a llama — the weakest object in the picture, and the
 *    one every judge looks at first. No mechanical check could see it. Opening
 *    the PNG and looking is the only thing that ever would.
 */
function drawUnicorn(tx, ty, ang, sc) {
  const bob = Math.sin(horn.bob) * 4;
  X.save();
  X.translate(tx, ty + bob);
  X.rotate(ang * .16);                  // she leans into the shaft
  if (sc) X.scale(sc, sc);

  const body = '#f6f1ff', shade = '#c9bce4', dark = '#a294c9';

  /* Legs first, so the barrel overlaps them. Far pair darker, so the animal
     has depth rather than four identical sticks. */
  X.lineCap = 'round';
  for (let i = 0; i < 4; i++) {
    ...
    /* Thicker at the top than the bottom, and finished with a hoof. Three
       widths and a dot, because the first version drew each leg as ONE
       constant-width stroke and the animal read as a body on wire — which is
       exactly the "hand-coded path is a different default, and a worse-looking
       one" that the art doctrine warns about. */
    X.lineWidth = far ? 9 : 11;
    const kx = hx + sw * 30 - Math.max(0, co) * 12, ky = hy + 54;
    X.beginPath();
    X.moveTo(hx, hy);
    /* Upper leg forward-back, lower leg folding — a gallop, not a pendulum. */
  ...
  /* Tail — rainbow strands, streaming west off the croup. */
  for (let i = 0; i < 7; i++) {
  ...
  /* One continuous silhouette: forehead -> muzzle -> jaw -> throat -> chest ->
     belly -> flank -> croup -> back -> neck crest -> poll. */
  X.fillStyle = body;
  X.beginPath();
  X.moveTo(-34, 36);
  X.bezierCurveTo(-23, 43, -13, 52, -7, 62);           // nose bridge to muzzle
  X.bezierCurveTo(-13, 71, -25, 72, -35, 67);          // muzzle and chin
  X.bezierCurveTo(-33, 79, -45, 85, -57, 93);          // jaw into the throat
  X.bezierCurveTo(-72, 102, -85, 114, -93, 129);       // throat to chest
  X.bezierCurveTo(-107, 133, -119, 135, -128, 139);    // chest into the belly
  X.bezierCurveTo(-150, 145, -170, 140, -181, 127);    // belly to the flank
  X.bezierCurveTo(-192, 116, -191, 99, -182, 90);      // croup
  X.bezierCurveTo(-161, 79, -128, 82, -105, 91);       // the back
  X.bezierCurveTo(-84, 73, -63, 55, -49, 40);          // crest of the neck
  X.bezierCurveTo(-45, 33, -38, 31, -34, 36);          // poll to forehead
  X.closePath(); X.fill();

  /* Shading down the near flank, so the barrel is round. */
  ...
  /* Ear, eye, nostril, and the line of the cheek. */
  X.fillStyle = shade;
  ...
  /* Mane — the same seven hues as the bands, in the same order, so the creature
     and the mechanic are visibly one object. A locked band is a dull strand. */
  for (let i = 0; i < 7; i++) {
  ...
  /* The horn: a tapering spiral to the origin, lit by what it is splitting. */
  ...
  /* The point of contact — a hard white star where the shaft breaks. */
...
function drawBlight(e) {
  ...
  /* A ragged blot, not a circle — polar radius with a fixed per-mote wobble. */
  ...
  X.closePath(); X.fill();
  /* Ember eyes — the only warm thing in it. */
...
function drawGeode(e) {
  if (e.seen <= .02) {
    /* Unrevealed: only a faint disturbance in the air. SCRY is the only way. */
...
/**
 * A meter, drawn ONLY when it has something to say.
 *
 * ⚠️ It used to draw unconditionally, and every un-grown bloom and every
 *    full-health blight carried an empty dark rectangle. Across a field of
 *    twenty entities that is twenty pieces of litter floating over the valley —
 *    clearly visible in shot_4 and invisible to every mechanical check, because
 *    litter is ink and ink is what a density gate rewards.
 */
...
/* ===========================================================================
   14. THE BANDS, DRAWN — additive wedges with a soft core, so seven of them
   overlapping read as light rather than as seven sticks.
   ========================================================================= */
function drawBands() {
  const R = reach();
  X.globalCompositeOperation = 'lighter';
  for (const i of S.bands) {
    ...
    /* Two passes: a wide soft halo, then a bright thin core. */
    ...
    /* The verb, riding its own band — this is the game's whole UI. */
    X.save();
  ...
  /* The empty slots — drawn as faint dashes so you can SEE what you are
     missing, and see the rainbow completing as you buy it. */
  for (let i = 0; i < 7; i++) {
...
function drawShaft() {
  ...
  X.beginPath(); X.moveTo(sun.x, sun.y); X.lineTo(horn.x, horn.y); X.stroke();
  /* The rising sun itself, hanging off the left edge. */
...
/* ===========================================================================
   15. HUD + SCREENS — all inside the canvas (see index.html for why).
   ========================================================================= */
...
function drawHUD() {
  ...
  /* Vigour: your meter for taking bites AND for paying for FLARE. */
  ...
  /* The spread readout, drawn as the comb itself. */
  const cx = 60, cy = H - 26;
...
function titleArt(cx, cy, len, a0, spr, al, lw) {
  /* A wordmark that IS the mechanic: the seven bands leaving one point.
     Parameterised because the two screens that use it need OPPOSITE
     compositions — the title converges at her horn and sprays across the sky,
     the Prism rises from below the panel as a wash. It used to be hard-coded
     to the title's geometry and reused verbatim on the Prism, where the fan
     went straight through the header text. */
  X.globalCompositeOperation = 'lighter';
  for (let i = 0; i < 7; i++) {
...
/* ---------------------------------------------------------------------------
   The shop used to score every upgrade with GREY SQUARES — five pips carrying
   none of this game's visual language, on the one screen whose whole job is to
   make you want the next unlock. Wing CLAUDE.md §3e names that exact failure
   and names the fix: compose the mark from the game's OWN renderer at the real
   data. So each row now draws a MINIATURE OF THE FAN, with the very property
   that upgrade buys already applied to it — REACH's bands are longer, POTENCY's
   are thicker, VIGOUR's guard is wider, RADIANCE and FLARE burn brighter. There
   is no second copy of the art, so it cannot drift from the game.
   ------------------------------------------------------------------------- */
function upMark(x, y, k, l) {
  X.save(); X.translate(x, y);
  X.globalCompositeOperation = 'lighter';
  /* Sized so the DIFFERENCE between two levels is legible at a glance — the
     first cut of this was ~30px wide and you had to hunt for it, which is the
     grey-pip problem again in a prettier costume. The one hard constraint is
     the 46px row pitch: a maxed REACH fan is 54 long at .125 spread, which is
     ±20 tall, so it cannot climb into its neighbour. */
...
function drawTitle() {
  ...
  /* She idles rather than freezes — the same gait the run uses, run off `t`. */
  horn.bob = t * 2.6; horn.gait = t * 4.4;
  /* Composed so the fan leaves HER HORN and sweeps up across the sky, which
     leaves the whole lower-right clear for type. The old title drew the fan
     from an empty point in mid-air, showed no unicorn at all, and put every
     word in the dark bottom half of the frame. */
...
function drawPrism() {
  X.drawImage(backdrop || paintValley(), 0, 0);
  /* ORDER MATTERS HERE AND IT WAS WRONG. The fan used to be drawn ON TOP of
     the scrim, in the title's own geometry, so it sprayed straight through the
     header — "next: WAVE n" was unreadable and the Light total was half lost,
     on the one screen where the player spends money. Drawing it BEHIND the
     scrim turns the same bands into a rainbow glow rising under the panel and
     makes every word legible, at no byte cost. */
  /* Converged well BELOW the frame, not just under its edge. At H+40 the
     origin was close enough that the bands entered the picture at full alpha,
     nearly parallel and 30px wide — a bright blob at the bottom fading to grey
     vertical curtains, which is not what a rainbow looks like. From H+160 they
     arrive already spreading and already softened, so colour pools under the
     shop and thins upward. */
  ...
  /* The band shop — the rainbow completing, priced. */
  const y = 420;
  ...
  /* The same mark as every other row, which is the point: this one shows the
     rainbow itself completing, dim slot by dim slot. */
...
/* Exposed ONLY so the headless playtest can drive and assert against the real
   simulation rather than against a copy of it. Costs a few bytes and is the
   only way a test can prove the game ran, not that a draw call did. */
self.PH = {
